...
QtObject {
  ...
  //   - the built-in bar option (`quickshell.bar`) is active when `bar.id` is
  //     missing or set to `quickshell.bar`.
  ...
  //     silently make `quickshell launch bar-settings` a no-op. Turning one off
  ...
  function isEnabled(id) {
    ...
    if (manifest) {
      if (Array.isArray(manifest.kinds) && manifest.kinds.indexOf("bar") !== -1) {
        ...
        if (!selectedBar) selectedBar = "quickshell.bar"
  ...
  function resolveEnabledId(id) {
    ...
    for (var candidate in installedPlugins) {
      ...
      var metadata = manifest && Util.isPlainObject(manifest.quickshell) ? manifest.quickshell : null
  ...
  function barTarget(config, placement, fallbackSection) {
    ...
    var anchors = { left: "quickshell.workspaces", center: "quickshell.weather", right: "quickshell.tray" }
  ...
  function activeCloneFor(config, sourceId) {
    for (var candidate in installedPlugins) {
      ...
      var candidateMetadata = candidateManifest && Util.isPlainObject(candidateManifest.quickshell)
        ? candidateManifest.quickshell : null
  ...
  function restoreCloneSource(config, cloneId, sourceId) {
    ...
    if (isBarOption) {
      if (sourceId === "quickshell.bar") delete config.bar.id
    ...
    shellConfigMutator(function(config) {
      ...
      if (isBarOption) {
        if (value) {
          ...
          if (clonedFrom && clonedFrom !== "quickshell.bar") config.bar.id = clonedFrom
  ...
  function parseScanOutput(text) {
    ...
    // `quickshell.*` namespace is reserved for built-ins, including bar widgets
