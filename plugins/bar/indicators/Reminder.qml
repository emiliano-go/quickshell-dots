...
BarIndicator {
  ...
  function openReminderFlow() {
    Quickshell.execDetached(["quickshell-reminder", "-i"])
  }
  ...
  Process {
    id: jsonProc
    command: ["quickshell-reminder", "show", "--json"]
  ...
  onPressed: function() {
    if (root.reminderCount > 0) Quickshell.execDetached(["quickshell-reminder", "show"])
