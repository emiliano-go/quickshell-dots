...
This is the Quickshell implementation of the Quickshell status bar. It is
shipped as a first-party plugin of [`quickshell-shell`](../../README.md), the
long-running shell host. The bar is mounted at startup and lives inside
the shell for its whole session.

- `manifest.json` declares the plugin (`id: quickshell.bar`, `kind: bar`) and points at `Bar.qml` as the entry point.
- `Bar.qml` is Quickshell-owned bar engine code, loaded by the quickshell-shell host. Users should not edit it directly.
...
- `quickshell bar position` updates only the user shell.json file.
...
```json
{
  ...
  "bar": {
    ...
    "layout": {
      "left": [
        { "id": "quickshell.menu" },
        { "id": "quickshell.spacer", "size": 12 },
        { "id": "quickshell.workspaces" }
      ],
      "center": [
        { "id": "quickshell.media" },
        { "id": "quickshell.clock", "format": "HH:mm" }
      ],
      "right": [
        { "id": "quickshell.audio" },
        { "id": "quickshell.power" }
...
| `quickshell.weather` | Weather icon + popup with forecast | left = popup · right = full notification |
| `quickshell.microphone` | Mic icon + scroll volume | left = mute toggle · middle = audio panel · scroll = source volume |

| `quickshell.audio` | Volume icon + popup with master slider, output-device picker, per-app mixer | left = popup · right = mute · middle = popup · scroll = volume |
| `quickshell.network` | Wi-Fi/Ethernet icon + popup with Wi-Fi scan, signal, connect, DNS provider selection | left = popup |
| `quickshell.tailscale` | Tailscale status, connection switcher, machine browser, and copy actions | left = popup · right = toggle · middle = refresh |
| `quickshell.agents` | AI coding agent limits with pace, today, last week, and all-time model breakdown | left = panel · right = launch agent · middle = next subscription |
| `quickshell.power` | Battery/AC icon + popup with battery stats, power profiles, and system info | left = popup · right = toggle percentage |
| `quickshell.bluetooth` | Bluetooth icon + popup with device list, connect/disconnect, battery | left = popup · right = toggle radio |
| `quickshell.monitor` | Brightness and laptop display controls | left = popup |

The `quickshell.indicators` widget loads individual bar indicators from `indicators/`. Omit `items` (or set it to an empty array) to show all indicators in the default order, or set `items` to a subset such as `["Dnd", "Reminder", "NightLight"]`. Set `alwaysShow` to `true` to keep inactive indicators visible instead of revealing them only on hover. Multiple `quickshell.indicators` instances are allowed, so different sections can show different subsets.
...
Item {
  ...
  MouseArea {
    anchors.fill: parent
    onClicked: if (bar) bar.run("quickshell-launch-or-focus-tui btop")
  }
}
```
...
`quickshell.menu` and `quickshell.media` declare their bar-widget entry points in their own
`manifest.json`. Bar layout ids are namespaced, e.g. `quickshell.audio`,
`quickshell.network`, and `quickshell.clock`.
...
and place third-party plugins with `quickshell-shell shell rescanPlugins`,
`quickshell plugin enable`, and `quickshell bar move`.
