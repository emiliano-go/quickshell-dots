...
Item {
  ...
  // The quickshell-shell host injects quickshellPath from QUICKSHELL_PATH.
  required property string quickshellPath
  ...
  // to "monospace"), so changing the system font (via `quickshell-font-set`)
  ...
  // Resolve the live bar-widget instance for a plugin id (e.g. "quickshell.bluetooth").
  ...
  // and the flag is created/removed by `quickshell-toggle-bar`.
  Process {
    id: barHiddenProbe
    running: true
