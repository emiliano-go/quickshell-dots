...
Item {
  ...
  property string quickshellPath: ""

  readonly property string home: Quickshell.env("HOME")
  readonly property string stateHome: home + "/.local/state"
  readonly property string userName: Quickshell.env("USER") || Quickshell.env("LOGNAME")
  readonly property string currentBackgroundLink: stateHome + "/quickshell/current/background"
  ...
  Process {
    ...
    command: ["bash", "-c", "if [[ -f /etc/pam.d/quickshell-lock-fingerprint ]] && command -v fprintd-list >/dev/null 2>&1 && fprintd-list \"$USER\" 2>/dev/null | grep -qi finger; then echo yes; else echo no; fi"]
  ...
  Process {
    ...
    command: ["bash", "-c", "quickshell-hyprland-session-locked"]
  ...
  Process {
    id: wakeProcess
    command: ["bash", "-c", "quickshell-system-wake"]
  }

  Process {
    id: blankProcess
    command: ["bash", "-c", "quickshell-brightness-keyboard off; quickshell-brightness-display off"]
  }
  ...
  FileView {
    path: "/etc/pam.d/quickshell-lock-password"
    watchChanges: true
    printErrors: false
