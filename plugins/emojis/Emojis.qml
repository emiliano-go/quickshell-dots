...
Item {
  ...
  function dismiss() {
    ...
    if (root.shell && typeof root.shell.hide === "function")
      root.shell.hide((root.manifest && root.manifest.id) || "quickshell.emojis")
  }
  ...
  function applySelected(emoji) {
    if (!emoji) return
    root.dismiss()
    Quickshell.execDetached([root.quickshellPath + "/bin/quickshell-menu-emoji-insert", emoji])
  }

  ListModel { id: displayModel }

  FileView {
    path: root.quickshellPath + "/shell/plugins/emojis/emojis.json"
