...
Panel {
  ...
  readonly property var mediaService: bar?.shell?.firstPartyServiceFor("quickshell.media")
  ...
  readonly property var candidateStreams: {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (String(n.name || "").indexOf("quickshell_speaker_tuning") === 0) continue
      list.push(n)
    }
    return list
  }
  ...
  //
  // quickshell-audio-output-sink resolves the *current* default output through any
  ...
  function showVolumeOsd(volume) {
    ...
    bar.shell.summon("quickshell.osd", JSON.stringify({
  ...
  function setDefaultSource(node) {
    ...
    if (node.id !== undefined && node.name) {
      Quickshell.execDetached([
        "quickshell-audio-input-set-default",
        String(node.id),
        String(node.name)
      ])
    }
  ...
  Process {
    ...
    command: ["quickshell-audio-sink-availability"]
  ...
  Process {
    id: volumeSinkProc
    command: ["quickshell-audio-output-sink"]
    stdout: StdioCollector {
      waitForEnd: true
