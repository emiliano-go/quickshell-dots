...
Panel {
  ...
  // The actual Bluetooth sequencing lives in bin/quickshell-bluetooth-device;
  ...
  function setDefaultAudioSink(sink) {
    ...
    if (sink.id !== undefined && sink.name) {
      Quickshell.execDetached([
        "quickshell-audio-output-set-default",
        String(sink.id),
        String(sink.name)
      ])
    }
  ...
  function deviceCommand(action, address) {
    return ["quickshell-bluetooth-device", action, address]
  }
  ...
  // disconnects, then removes the BlueZ pairing record via quickshell-bluetooth-device.
  ...
  // the adapter came back on at the next boot. quickshell-bluetooth-power moves the
  ...
  function toggleBluetooth() {
    if (!adapter) return
    Quickshell.execDetached(["quickshell-bluetooth-power", adapter.enabled ? "off" : "on"])
  }

  IpcHandler {
    target: "quickshell.bluetooth"
