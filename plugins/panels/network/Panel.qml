...
Panel {
  ...
  // `quickshell-network-status --verbose` samples (~1.5s apart via detailsPoll).
  ...
  // Wi-Fi band state from `quickshell-network-band`. `bandCurrent` is the band
  ...
  // The share card is its own panel plugin (quickshell.wifiqr) so a replacement
  ...
  function summonWifiQr(forceDetect) {
    ...
    bar.shell.summon("quickshell.wifiqr", JSON.stringify(payload))
  }
  ...
  function refresh(scanWifi) {
    ...
    if (!bandProc.running) {
      bandProc.command = ["quickshell-network-band"]
  ...
  function setBand(band) {
    ...
    actionProc.command = ["quickshell-network-band", band]
    actionProc.running = true
  }

  // The speed test is its own panel plugin (quickshell.speedtest) so a
  ...
  function summonSpeedTest() {
    ...
    bar.shell.summon("quickshell.speedtest", connection ? JSON.stringify({ connection: connection }) : "{}")
  }

  function dnsCommand(provider) {
    var command = "quickshell-dns"
  ...
  function setDns(provider) {
    ...
    if (provider === "Custom") {
      var launcher = "quickshell-launch-floating-terminal-with-presentation"
  ...
  Process {
    id: detailsProc
    command: ["quickshell-network-status", "--verbose"]
