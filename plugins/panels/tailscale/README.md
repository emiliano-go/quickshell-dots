# Tailscale Quickshell Widget

Native Quickshell bar widget for Tailscale.
...
`quickshell-tailscale-receive` service, which announces each one with a
notification (an image preview when the file is an image, and a click to open
it). The Tailscale service install enables it; `quickshell tailscale receive`
...
This widget ships as first-party plugin `quickshell.tailscale`. Add it with `quickshell plugin enable quickshell.tailscale`, then place it with `quickshell bar move quickshell.tailscale` if desired.
