...
Panel {
  ...
  function setBrightness(value) {
    ...
    setBrightnessProc.command = ["quickshell-brightness-display", "--no-osd", "--monitor", root.focusedMonitor, percent + "%"]
  ...
  function setScale(scale) {
    actionProc.command = ["bash", "-c", "quickshell-hyprland-monitor-scaling " + scale]
    if (!actionProc.running) actionProc.running = true
  }
  ...
  function setTextSize(px) {
    textScaleProc.command = ["quickshell-display-text-size", String(px)]
  ...
  Process {
    id: stateProc
    command: ["quickshell-monitor-state"]
