...
Item {
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      // Walker-style fallback: a query that matches nothing becomes a web
      // search in the default browser, so Enter always does something useful.
      // encodeURIComponent escapes single quotes, keeping the shell command safe.
      if (rows.length === 0) {
        var webSearchRow = root.displayRow({
          id: "__websearch__",
          parent: active,
          kind: "action",
          icon: "󰍉",
          ...
          target: "",
          description: "",
          action: "xdg-open 'https://www.google.com/search?q=" + encodeURIComponent(query) + "'",
          provider: "",
          aliases: [],
          when: "",
          checked: "",
