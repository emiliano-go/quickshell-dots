...
// against `quickshell-default-browser` -- so the batch runs it once and the rows
...
var GUARD_READERS = [
  "quickshell-channel-current",
  "quickshell-default-agent",
  "quickshell-default-browser",
  "quickshell-default-editor",
  "quickshell-default-terminal",
  "quickshell-dns"
]
...
function guardHelpers() {
  return 'declare -A __quickshell_pkgs=()\n'
    + 'mapfile -t __quickshell_pkg_names < <({ pacman -Qq; LC_ALL=C pacman -Qi'
    + " | awk '/^[A-Za-z]/ { provides = ($0 ~ /^Provides/); sub(/^[^:]*: /, \"\") }"
    + ' provides && $0 != "None" { n = split($0, p, " ");'
    + ' for (i = 1; i <= n; i++) { sub(/[<>=].*/, "", p[i]); print p[i] } }\'; } 2>/dev/null)\n'
    + 'for __quickshell_pkg in "${__quickshell_pkg_names[@]}"; do __quickshell_pkgs[$__quickshell_pkg]=1; done\n'
    + '__quickshell_pkg_has() { [[ -n ${__quickshell_pkgs[$1]-} ]] && return 0; '
    + '[[ $1 == *[\\<\\>=]* ]] && { pacman -Q "$1" &>/dev/null; return; }; return 1; }\n'
    + 'quickshell-pkg-present() { local p; for p in "$@"; do __quickshell_pkg_has "$p" || return 1; done; return 0; }\n'
    + 'quickshell-pkg-missing() { local p; for p in "$@"; do __quickshell_pkg_has "$p" || return 0; done; return 1; }\n'
    + 'quickshell-cmd-present() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 1; done; return 0; }\n'
    + 'quickshell-cmd-missing() { local c; for c in "$@"; do command -v "$c" &>/dev/null || return 0; done; return 1; }\n'
}
...
function guardPrelude(guards) {
  ...
  for (var i = 0; i < GUARD_READERS.length; i++) {
    ...
    prelude += "__quickshell_read_" + i + "=$(" + GUARD_READERS[i] + " 2>/dev/null) || :\n"
  }
...
function guardReaderSlot(index) {
  return "${__quickshell_read_" + index + "}"
}
