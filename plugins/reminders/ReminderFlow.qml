...
Item {
  ...
  function submit() {
    ...
    if (root.step === "minutes") {
      ...
      if (!nextMinutes) {
        Quickshell.execDetached([root.quickshellPath + "/bin/quickshell-notification-send", "Invalid reminder", "Enter the number of minutes"])
        return
      }
    ...
    if (root.step === "message") {
      var args = [root.quickshellPath + "/bin/quickshell-reminder"].concat(ReminderFlowModel.reminderArgs(root.minutes, selection))
      root.dismiss()
