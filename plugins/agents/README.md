...
`quickshell-agent-usage-update` writes to `~/.local/state/quickshell/agents/usage/`
...
`quickshell plugin disable quickshell.agents`.

## Data

Each agent is one JSON record in `~/.local/state/quickshell/agents/usage/`,
written by `quickshell-agent-usage-update`. That command runs one
`quickshell-agent-usage-<agent>` collector per agent; the widget invokes it
...
- IPC: `quickshell-shell quickshell.agents <open|close|toggle|refresh|next>`.

## Settings

Settings live in the widget's entry in `~/.config/quickshell/shell.json`. The
top-level keys can be set with
`quickshell bar set quickshell.agents <key> <value>`:
...
```bash
quickshell bar set quickshell.agents refreshIntervalSec 300 --json
quickshell bar set quickshell.agents syncDir '~/Sync/agent-usage'
```
...
```bash
quickshell bar set quickshell.agents providers '{
  "claude": { "enabled": true },
  "codex": { "enabled": false },
