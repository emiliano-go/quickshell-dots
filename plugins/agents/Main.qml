...
// quickshell-agent-usage-update, which writes one JSON record per agent into
...
Item {
  ...
  function updateCommand(kind, agentIds) {
    var command = ["quickshell-agent-usage-update"]
  ...
  // Snapshots keep the field names older Quickshell versions wrote, so a fleet
