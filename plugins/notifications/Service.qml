// Notification service for the quickshell shell.
...
Item {
  ...
  // Toasts are fixed to the top-right corner. They only clear the quickshell bar
  ...
  // Run the popup's click action, then dismiss. Quickshell's own toasts carry the
  ...
  function focusApp(entry) {
    ...
    focusAppProc.command = [
      service.quickshellPath + "/bin/quickshell-hyprland-focus-app",
      String(entry.app)
    ]
  ...
  // toasts survive shell restarts (notably the restart `quickshell-update`
